...
class FlightPathPhase(PhaseBuilderBase):
    ...
    def build_phase(self, aviary_options: AviaryValues = None):
        ...
        phase.add_state(
            Dynamic.Mission.FLIGHT_PATH_ANGLE, fix_initial=fix_initial, fix_final=False,
            lower=-np.pi/6., upper=np.pi/6., ref=1.0, defect_ref=1.0, units='rad',
    ...
    def _extra_ode_init_kwargs(self):
        ...
        return {
            ...
            'alpha_mode': self.user_options.get_val('alpha_mode'),
            'input_speed_type': SpeedType.MACH,
            'ground_roll': self.user_options.get_val('ground_roll'),
            'clean': self.user_options.get_val('clean'),
            'solve_for_throttle': self.user_options.get_val('solve_for_throttle'),
        }


FlightPathPhase._add_meta_data(
...
FlightPathPhase._add_meta_data('polynomial_control_order', val=3)

FlightPathPhase._add_meta_data('use_polynomial_control', val=True)

FlightPathPhase._add_meta_data('add_initial_mass_constraint', val=False)

FlightPathPhase._add_meta_data('fix_initial', val=True)

FlightPathPhase._add_meta_data('ground_roll', val=True)

FlightPathPhase._add_meta_data('clean', val=True)

FlightPathPhase._add_meta_data('fix_duration', val=False)

FlightPathPhase._add_meta_data('optimize_mach', val=False)

FlightPathPhase._add_meta_data('optimize_altitude', val=False)

FlightPathPhase._add_meta_data('initial_bounds', val=(0., 100.), units='s')

FlightPathPhase._add_meta_data('duration_bounds', val=(0., 100.), units='s')
...
FlightPathPhase._add_meta_data(
    'solve_for_throttle', val=True, desc='solve for throttle setting')

FlightPathPhase._add_meta_data('constrain_final', val=False)

FlightPathPhase._add_meta_data('input_initial', val=False)

FlightPathPhase._add_meta_data('initial_mach', val=None, units='unitless')

FlightPathPhase._add_meta_data('final_mach', val=None, units='unitless')

FlightPathPhase._add_meta_data('initial_altitude', val=None, units='ft')

FlightPathPhase._add_meta_data('final_altitude', val=None, units='ft')

FlightPathPhase._add_meta_data('mach_bounds', val=(0., 2.), units='unitless')

FlightPathPhase._add_meta_data('altitude_bounds', val=(0., 60.e3), units='ft')

FlightPathPhase._add_meta_data('alpha_mode', val=AlphaModes.DEFAULT)

FlightPathPhase._add_meta_data('constraints', val={})

FlightPathPhase._add_initial_guess_meta_data(
    InitialGuessTime('time'),
