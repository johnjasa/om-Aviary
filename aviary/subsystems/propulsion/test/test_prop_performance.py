...
class PropPerformanceTest(unittest.TestCase):
    def setUp(self):
        ...
        pp.set_input_defaults(Dynamic.Mission.VELOCITY, 0 *
    ...
    def compare_results(self, case_idx_begin, case_idx_end):
        ...
        for case_idx in range(case_idx_begin, case_idx_end):
            assert_near_equal(cthr[case_idx - case_idx_begin],
                              CT[case_idx], tolerance=tol)
            assert_near_equal(ctlf[case_idx - case_idx_begin],
                              XFT[case_idx], tolerance=tol)
            assert_near_equal(tccl[case_idx - case_idx_begin],
                              CTX[case_idx], tolerance=tol)
            assert_near_equal(angb[case_idx - case_idx_begin],
                              three_quart_blade_angle[case_idx], tolerance=tol)
            assert_near_equal(thrt[case_idx - case_idx_begin],
                              thrust[case_idx], tolerance=tol)
            assert_near_equal(peff[case_idx - case_idx_begin],
                              prop_eff[case_idx], tolerance=tol)
            assert_near_equal(lfac[case_idx - case_idx_begin],
                              install_loss[case_idx], tolerance=tol)
            assert_near_equal(ieff[case_idx - case_idx_begin],
                              install_eff[case_idx], tolerance=tol)
