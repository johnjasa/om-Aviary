...
keywords = ["Axial", "Aero", "OutRunner", "Development"] # user input specifies what keyword to use. Must be a list, even if there is only one keyword
motor_power = 500. # user input specifies the desired power in units of kW
...
class MotorWeight(om.ExplicitComponent): # This component calculates a linear regression and its derivatives for the power and weight of the desired motors.

    def initialize(self):
        self.options.declare("keywords", default=["Axial"], types=list, desc="keywords to use in component")
        self.options.declare("num_motors", default=4, desc="number of motors to calculate the weight of")

    def setup(self):
        #get data and compute regression
        ...
        #inputs
        add_aviary_input(self, Aircraft.Motor.POWER_MAX, val=100.0, meta_data=ExtendedMetaData)

        #outputs
        add_aviary_output(self, Aircraft.Motor.MASS, val=1.0, meta_data=ExtendedMetaData)

        #partials
        self.declare_partials(Aircraft.Motor.MASS, Aircraft.Motor.POWER_MAX)

    def compute(self, inputs, outputs):
        outputs[Aircraft.Motor.MASS] = np.multiply(self.options["num_motors"], np.add(np.multiply(self.coefficients[0], inputs[Aircraft.Motor.POWER_MAX]), self.coefficients[1]))

    def compute_partials(self, inputs, J):
        J[Aircraft.Motor.MASS, Aircraft.Motor.POWER_MAX] = self.coefficients[0] * self.options["num_motors"]